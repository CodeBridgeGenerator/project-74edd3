...
const ProjectCreateDialogComponent = (props) => {
    ...
    const validate = () => {
        ...
        const error = {};
            ...
            if (_.isEmpty(_entity?.auth)) {
                error["auth"] = `Auth field is required`;
                ret = false;
            }
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            auth: _entity?.auth,description: _entity?.description,
    ...
    return (
        <Dialog header="Create Project" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            ...
            <span className="align-items-center">
                ...
                <InputText id="auth" className="w-full mb-3 p-inputtext-sm" value={_entity?.auth} onChange={(e) => setValByKey("auth", e.target.value)}  required  />
            </span>
            <small className="p-error">
            {!_.isEmpty(error["auth"]) ? (
              <p className="m-0" key="error-auth">
                {error["auth"]}
              </p>
            ) : null}
          </small>
